...
<a id="common_arithmetic_opcodes"></a>
...
<a id="common_environment_opcodes"></a>
...
**World State**
: 160位地址标识符和账户状态之间的映射，在不可变的_Merkle Patricia Tree_数据结构中维护。
...
<a id="compiling_solidity_to_evm"></a>
...
<a id="solc_asm_option"></a>
```
$ solc -o BytecodeOutputDir --asm Example.sol
```

<a id="solc_bin_option"></a>
...
生成的输出操作码文件将取决于Solidity源文件中包含的特定合约。我们的简单Solidity文件_Example.sol_ [simple_solidity_example](#simple_solidity_example)只有一个名为“example”的合约。

<a id="simple_solidity_example"></a>
```
...
如果查看_BytecodeDir_目录，你将看到操作码文件_example.opcode_（请参阅[simple_solidity_example](#simple_solidity_example)），其中包含“example”合约的EVM机器语言操作码指令。在文本编辑器中打开_example.opcode_文件将显示以下内容：

<a id="opcode_output"></a>
```
...
让我们检查前两条指令（参考[common_stack_opcodes](#common_stack_opcodes)）：

<a id="opcode_analysis_1"></a>
```
...
<a id="opcode_analysis_3"></a>
```
MSTORE CALLVALUE
```

MSTORE是一个堆栈/内存操作（参见[common_stack_opcodes](#common_stack_opcodes)），它将值保存到内存中，而CALLVALUE是一个环境操作码（参见[common_environment_opcodes](#common_environment_opcodes)），它返回正在执行的消息调用的存放值。

<a id="evm_bytecode_execution"></a>

#### 执行EVM字节码


<a id="gas_accounting_execution"></a>
...
<a id="turing_completeness_and_gas"></a>
...
<a id="bytecode_vs_runtime_bytecode"></a>
...
<a id="disassembling_the_bytecode"></a>
...
<a id="Faucet_disassembled"></a>
![Faucet.sol runtime bytecode disassembled](images/Faucet_disassembled.png)

当你将交易发送到智能合约时，交易首先会与该智能合约的**调度员（dispatcher）**进行交互。调度程序读入交易的数据字段并将其发送到适当的函数。

在熟悉的MSTORE指令之后，我们在编译的Faucet.sol合约中看到以下创建：

<a id="faucet_instructions"></a>
```
PUSH1 0x4
CALLDATASIZE
LT
...
<a id="faucet_function_identifier"></a>
```
...
<a id="faucet_jump_instruction_text"></a>
```
...
<a id="Faucet_jumpi_instruction"></a>
![JUMPI instruction leading to fallback function](images/Faucet_jumpi_instruction.png)
...
<a id="faucet_calldataload_instruction_text"></a>
```
...
.Current stack
[width="40%",frame="topbot",options="header,footer"]
| Stack | 32 bytes of calldata starting at byte 0 |
| --- | --- |
...
.Current stack
[width="40%",frame="topbot",options="header,footer"]
| Stack | 0x1000000... (29 bytes in length) |
| --- | --- |

下一条指令是“DIV”，其工作方式如下：

<a id="faucet_div_instruction_text"></a>
```
div(x, y) // x / y
```
...
.Current stack
[width="40%",frame="topbot",options="header,footer"]
| Stack | function identifier sent in msg.data |
| --- | --- |
...
| Stack | function identifier sent in msg.data |
| --- | --- |
| function identifier sent in msg.data | 0x2e1a7d4d |
