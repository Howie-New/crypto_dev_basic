[上一章：开发工具，框架和库](Chapter_10.md)

<a id="Tokens_chapter"></a>

## Tokens


<a id="Tokens_definition"></a>
...
严格地说，如果可以跟踪Token的历史出处，那么它就不是完全可替代的。追踪出处的能力可能导致黑名单和白名单，从而降低或消除互通性。我们将在[privacy](#privacy)中进一步研究。

不可互换的Token是代表独特的有形或无形商品的Token，因此不可互换。例如，表示_特定的_ Van Gogh绘画所有权的Token不等同于代表毕加索的另一个Token。同样，表示特定数字收藏的Token，如特定的CryptoKitty（请参阅[cryptoKitties](#cryptoKitties)）与任何其他CryptoKitty都不可互换。每个不可互换的Token与唯一标识符相关联，例如序列号。

本节后面我们将看到可替换和不可替换Token的例子。

<a id="counterparty_risk"></a>
...
<a id="who_needs_utility_Tokens"></a>
...
<a id="ERC20_interface_example"></a>
```
contract ERC20 {
...
<a id="balance_mapping"></a>
.Balances: a mapping from address (owner) to amount (balance)
```
mapping(address => uint256) balances;
```

第二个数据结构是配额的data mapping。正如我们将在[transfer_workflows](#transfer_workflows)中看到的那样，使用ERC20Token，Token的所有者可以将权限委托给花钱者，允许他们从所有者的余额中花费特定金额（配额）。ERC20合同通过二维映射追踪配额，主关键字是Token所有者的地址，映射到一个花费者地址和配额金额：

<a id="allowance_mapping"></a>
...
<a id="approve_transferFrom_workflow"></a>
![The two-step approve &amp; transferFrom workflow of ERC20 Tokens](images/approve_transferFrom_workflow.png)
...
<a id="METoken_example"></a>

#### 发布我们自己的ERC20Token


让我们创建并发布我们自己的Token。在这个例子中，我们将使用`truffle` 框架（参见[truffle](#truffle)）。该示例假设你已经安装了`truffle`，进行了配置，并熟悉其基本操作。
...
首先，让我们创建并初始化一个Truffle项目目录，就像我们在[truffle_project_directory](#truffle_project_directory)中所做的那样。运行这四个命令并接受任何问题的默认答案：

<a id="initialize_truffle"></a>
```
$ mkdir METoken
$ cd METoken
...
在我们部署其中一个以太坊测试网络之前，让我们开始一个本地区块链来测试一切。正如我们在[using_ganache](#using_ganache)中所做的那样，从 `ganache-cli` 的命令行或从图形用户界面启动 `ganache` 区块链。

一旦 `ganache` 启动，我们就可以部署我们的METoken合约，看看是否一切都按预期工作：

<a id="deploy_METoken"></a>
```
...
<a id="ganache_METoken"></a>
![METoken deployment on Ganache](images/ganache_meToken.png)

<a id="truffle_console"></a>
...
<a id="ganache_METoken_transfer"></a>
![METoken transfer on Ganache](images/ganache_meToken_transfer.png)

<a id="sending_erc20_Tokens_contracts"></a>
...
对于这个例子，我们制作了Truffle项目目录的副本，将其称为 `METoken_METFaucet`，初始化Truffle，npm，安装OpenZeppelin依赖项并复制`METoken.sol`合同。有关详细说明，请参阅我们的第一个示例[METoken_example](#METoken_example)。
...
[下一章：去中心化应用 (DApps)](Chapter_11.md)
