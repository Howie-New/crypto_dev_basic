...
交易消息的结构使用递归长度前缀（RLP）编码方案（参见 [rlp](#rlp) ）进行序列化，该方案是专门为以太坊中准确和字节完美的数据序列化而创建的。以太坊中的所有数字都被编码为大端序整数，其长度为8位的倍数。
...
nonce是交易中最重要和最少被理解的组成部分之一。黄皮书中的定义（见 [yellow_paper](#yellow_paper) ）写道：
...
<a id="nonce_getTransactionCount"></a>
...
> **提示:**
> 该nonce是一个基于零的计数器，意味着第一个交易的nonce是0.在 [nonce_getTransactionCount](#nonce_getTransactionCount)中，我们有一个交易的计数为40，这意味着从0到39nonce已经被看到。下一个交易的nonce将是40。
...
我们在 [gas](#gas) 中详细讨论_gas_。但是，让我们介绍有关交易的 `gasPrice` 和 `startGas` 字段的一些基本知识。
...
<a id="calc_gas_price"></a>
与gas有关的第二个重要领域是 `startGas`。这在 [gas](#gas) 中有更详细的解释。简单地说，`startGas` 定义交易发起人愿意花费多少单位完成交易。对于简单付款，意味着将ether从一个EOA转移到另一个EOA的交易，所需的gas量固定为21,000个gas单位。要计算需要花费多少ether，你需要将你愿意支付的 `gasPrice` 乘以21,000：
...
发送一个交易到一个无效的地址会_燃烧_发送的ether，使其永远不可访问（不可花费），因为不能生成用来使用它的签名。假定地址验证发生在用户界面级别（参见 [eip-55](#eip-55) 或 [icap](#icap)）。事实上，有很多合理的理由来燃烧ether，包括作为游戏理论，来抑制支付通道和其他智能合约作弊。

<a id="tx_value_data"></a>
...
<a id="tx_value_nodata"></a>

##### 有value的交易（支付），没有data


<a id="tx_value_nodata_src"></a>
.Value, no data
```javascript
...
<a id="parity_txdemo_value_nodata"></a>
![Parity wallet showing a transaction with value, but no data](images/parity_txdemo_value_nodata.png)
...
<a id="parity_txdemo_value_data"></a>
![Parity wallet showing a transaction with value and data](images/parity_txdemo_value_data.png)


<a id="tx_novalue_nodata"></a>

##### 0 value 的交易，只有数据


<a id="tx_novalue_nodata_src"></a>
.No value, only data
```javascript
...
<a id="parity_txdemo_novalue_data"></a>
![Parity wallet showing a transaction with no value, only data](images/parity_txdemo_novalue_data.png)


<a id="tx_novalue_data"></a>
...
<a id="parity_txdemo_novalue_nodata"></a>
![Parity wallet showing a transaction with no value, and no data](images/parity_txdemo_novalue_nodata.png)

<a id="value_EOA_contracts"></a>
...
如果目标地址（`to`）是合约，则EVM将执行合约并尝试调用你的交易的 `data` 中指定的函数（参见 [invocation](#invocation) ）。如果你的交易中没有 `data`，那么EVM将调用目标合约的 _fallback_ 函数，如果该函数是payable，则将执行该函数以确定下一步该做什么。
...
<a id="withdraw_function_src"></a>
我们来看一个简单的例子，它来自我们的[solidity_faucet_example](#solidity_faucet_example)。在`Faucet.sol`中，我们为取款定义了一个函数：
...
尽管零地址仅用于合约注册，但它有时会收到来自各个地址的付款。对此有两种解释：无论是偶然的，导致ether的丧失，还是故意的_ ether燃烧_（见[burning_ether](#burning_ether)）。如果你想进行有意识的ether燃烧，你应该向网络明确你的意图，并使用专门指定的燃烧地址：
...
作为例子，我们可以发布 [intro](#intro) 中使用的 `Faucet.sol`。合约需要编译成二进制十六进制表示。这可以用Solidiy编译器完成。
...
<a id="publish_contract_from_web3"></a>
![Etherscan showing the contract successully mined](images/contract_published.png)
...
在这里我们可以看到合约的地址。我们可以按照 [data_EOA](#data_EOA) 所示，从合约发送和接收资金。
...
<a id="publish_contract_transactions"></a>
![Etherscan showing the transactions for sending and receiving funds](images/published_contract_transactions.png)
...
以太坊中使用的数字签名算法是_Elliptic Curve Digital Signature Algorithm_，或_ECDSA_。ECDSA是用于基于椭圆曲线私钥/公钥对的数字签名的算法，如 [elliptic_curve](#elliptic_curve) 中所述。
...
<a id="digital_signature_definition"></a>
...
更多关于ECDSA数学的细节可以在 [ecdsa_math](#ecdsa_math) 中找到。

<a id="sign_function"></a>
...
从_q_和_G_开始，我们生成相应的临时公钥_Q_（以_Q = q * G_计算，与以太坊公钥的派生方式相同，参见[pubkey](#pubkey)）。数字签名的_R_值就是临时公钥_Q_的x坐标。
...
> **提示:**
> 在＃2,675,000块，Ethereum实施了“Spurious Dragon”硬分叉，除其他更改外，还推出了包括交易重播保护的新签名方案。这个新的签名方案在EIP-155中指定（参见[eip155](#eip155)）。此更改会影响签名过程的第一步，在签名之前向交易添加三个字段（v，r，s）。
...
如[tx_struct](#tx_struct)所述，交易消息不包含任何“from”字段。这是因为发起者的公钥可以直接从ECDSA签名中计算出来。一旦你有公钥，你可以很容易地计算出地址。恢复签名者公钥的过程称为_公钥恢复_。

给定 [ecdsa_math](#ecdsa_math) 中计算的值 `r` 和 `s`，我们可以计算两个可能的公钥。
...
一旦交易被签署，它就可以传送到以太坊网络。创建，签署和广播交易的三个步骤通常发生在单个函数中，例如使用`web3.eth.sendTransaction`。但是，正如我们在[raw_tx](#raw_tx)中看到的那样，你可以通过两个单独的步骤创建和签署交易。一旦你签署了交易记录，你就可以使用`web3.eth.sendSignedTransaction`传输该交易记录，该方法采用十六进制编码的签名交易信息并在Ethereum网络上传输。
...
虽然以太坊中的所有节点都是相同的对等节点，但其中一些节点由_矿工_操作，并将交易和数据块提供给_挖矿农场_，这些节点是具有高性能图形处理单元（GPU）的计算机。挖掘计算机将交易添加到候选块，并尝试查找使得候选块有效的_Proof-of-Work_。我们将在[consensus](#consensus)中更详细地讨论这一点。

不深入太多细节，有效的交易最终将被包含在一个交易块中，并记录在以太坊区块链中。一旦开采成块，交易还通过修改账户余额（在简单付款的情况下）或通过调用改变其内部状态的合约来修改以太坊单例的状态。这些更改将与交易一起以交易_收据_ _receipt_ 的形式记录，该交易也可能包含_事件_ _events_。我们将在 [evm](#evm) 中更详细地检查所有这些。
